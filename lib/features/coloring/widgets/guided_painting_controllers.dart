...
class DrawingStepController extends ChangeNotifier {
  ...
  bool get isOutlineComplete => true;
  ...
  bool isRegionCompleted(String regionId) => true;
  ...
  List<DrawingPartStep> _buildParts(LevelModel level) {
    ...
    for (int index = 0; index < level.regions.length; index++) {
...
class ColoringStepController extends ChangeNotifier {
  ...
  bool get isLevelComplete {
    if (_orderedRegionIds.isEmpty) return false;
    for (final regionId in _orderedRegionIds) {
      final progress = _fillProgress[regionId] ?? 0.0;
      if (progress < 0.60) return false;
    }
    return true;
  }
  ...
  void setActiveRegionId(String? regionId) {
  ...
  void setRegionProgress(String regionId, double progress) {
    _fillProgress[regionId] = progress.clamp(0.0, 1.0);
    if (progress >= completionThreshold) {
      _filledRegionIds.add(regionId);
    } else {
      _filledRegionIds.remove(regionId);
    }
    notifyListeners();
  }
  ...
  Offset? handlePaintUpdate({
    ...
    if (strokes.isEmpty) {
      ...
      strokes.last.points.addAll(interpolatedPoints);
    }

    _lastPaintPoint = targetPoint;
    _registerCoverage(
      regionId: regionId,
      path: path,
      points: interpolatedPoints,
